...
def main(filename):
    ...
    while True:
        ...
        if ret:
            ...
            if len(corners) > 0:
                list_ids = ",".join(map(str, np.ravel(ids)))            # idsを一次元化し、数値から文字列にし、各要素を連結する
                cv2.putText(frame, f"marker is found: id={list_ids}", (20, 100), cv2.FONT_HERSHEY_DUPLEX, 1, (0,0,255), 2)

                corners = corners[0].reshape((4,2))                     # cornersをリシェイプする
                cx, cy = np.mean(corners, axis=0)                       # 各点のxとyの平均値
                cx, cy = int(cx), int(cy)                               # 整数にする
                cv2.circle(frame, (cx, cy), 3, (0,255,0), 2)
                roi.draw(frame, cx, cy)
                # cv2.rectangle(frame, (cx-roi.width//2, cy+roi.y1), (cx+roi.width//2, cy+roi.y2), (0,0,255), 1)
                """
                points, roi.image = find_rect(roi.image, cx, cy)
                if len(points) > 0:
                    cv2.putText(frame, f"rect is found: pos={points[0]}", (20, 150), cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0), 1)
                else:
                    cv2.putText(frame, f"rect is not found", (20, 150), cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0), 1)
                """
#                roi.image = find_triangle(roi.image)
                wait_time = 100                                           # cv2.imshow()の待ち時間
            else:
                cv2.putText(frame, f"marker is not found", (20, 100), cv2.FONT_HERSHEY_DUPLEX, 1, (255,255,255), 2)
                wait_time = 100                                           # cv2.imshow()の待ち時間
