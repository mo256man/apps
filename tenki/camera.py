...
class Camera():
    ...
    def read_data(self):
        ...
        frame = cv2.putText(frame, now.strftime("%m/%d %H:%M"), (10, 40), cv2.FONT_HERSHEY_DUPLEX, 1, color, 2)
        ...
        """
        # ここからが本番　7セグを読み取る
            str_now = now.strftime("%Y-%m-%d %H:%M")                    # 現在時刻を文字列にする
        ...
        print(now)
        return json.dumps({"image1":str_img1})
